...
exports.initiatePayment = async(req, res) => {
    ...
    try {
        ...
        const cart = await prisma.cart.findUnique({ where: { userId: user.id }, include:{ cartItems: true }})
        ...
        const paymentData = {
          ...
          customer: {
            ...
            name: users.firstName || firstName,
            phonenumber: users.phone || phone,
          },
          meta: {
            firstName,
            lastName,
        ...
        if(data.status === "success"){
            return res.status(200).json({ success: true, message: "Payment successful", data: data.data.link, orderId})
        }else {
...
exports.verifyPayment = async(req, res) => {
    ...
    try {
        ...
        if(data.status === "success"){
            const cart = await prisma.cart.findUnique({ where: { userId: req.user.id }, include: { cartItems: true }})
            if(!cart) return res.status(400).json({ success: false, message: "Cart not found" })

            if (cart.cartItems.length === 0) return res.status(400).json({ success: false, message: "Cart is empty" });

            const orders = await prisma.order.create({ data: {
            ...
            console.log(orders)
            return res.status(201).json({ success: true, message: "Order created successfully", data: orders })
        }else{
